...
#### 下载Krescue镜像
Krescue的镜像可以从[这里下载](/Firmware/Krescue/dump/),选择你相应的板子的固件。
![Kerscure_downloads](/images/vim3/Krescue_downloads.png)

#### 制作启动盘
将mircoSD卡插进PC上。将下载的镜像烧录进SD卡。
![Make_Krescue](/images/vim3/HowtoUseEtcher.png)


### 运行
将制作好的SD卡插入板子,连接HDMI，进入[升级模式](/vim3/HowtoBootIntoUpgradeMode.html)，看到下图就是成功启动了kerscure
![boot Krescue](/images/vim3/krescue_boot.jpg)
...
```
你会看到可用的固件列表
![krescue firmware list](/images/vim3/krescue_list.png)
```
$ curl -sfL dl.khadas.com/.mega | sh -s - [BOARD_ID] > /dev/mmcblk0 (或者`/dev/mmcblk1`)
```
等待烧录完成，镜像就被写入了SD卡里，此时就可以进入[升级模式](/vim3/HowtoBootIntoUpgradeMode.html)启动krescue了。
